...
import com.sol.app.myPage.dao.MySmallClubApplyDAO;

public class MySmallClubApplyOkController implements Execute {

	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("MySmallClubApplyOkController 진입");
		...
		MySmallClubApplyDAO mySmallClubApplyDAO = new MySmallClubApplyDAO();
		...
		if (memberNumber == null) {
			...
			int total = mySmallClubApplyDAO.getTotal(memberNumber);
			...
			mySmallClubApplyDAO.selectList(map).stream().map(gson::toJson).map(JsonParser::parseString).forEach((data) -> {
				//목록 행 정보
				JsonArray a = new JsonArray();

				a.add(data.getAsJsonObject().get("smallClubTitle"));
				a.add(data.getAsJsonObject().get("memberNickname"));
				a.add(data.getAsJsonObject().get("smallClubStartDate"));
				...
				String href = "/club/smallClubDetailOk.cl?smallClubNumber=" + data.getAsJsonObject().get("smallClubNumber");
				etc.addProperty("href", href);
				
				etcArr.add(etc);
			});

			obj.addProperty("listTitle", "나의 신청 소모임 내역");
			obj.add("cols", cols);
			obj.add("list", list);
