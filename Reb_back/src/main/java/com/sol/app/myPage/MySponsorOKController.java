...
public class MySponsorOKController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("MySponsorOKController 진입");
		...
		if (memberNumber == null) {
			...
			int total = mySponsorDAO.getTotal(memberNumber);

			System.out.println("MySponsorOKController getTotal 끝");
			...
			mySponsorDAO.selectList(map).stream().map(gson::toJson).map(JsonParser::parseString).forEach((data) -> {
				JsonArray a = new JsonArray();

				a.add(data.getAsJsonObject().get("sponsorAmount").getAsString() + " 원");
				a.add(data.getAsJsonObject().get("sponsorDate"));
				
				list.add(a);
			});

			obj.addProperty("listTitle", "나의 후원 내역");
			obj.add("cols", cols);
			obj.add("list", list);
