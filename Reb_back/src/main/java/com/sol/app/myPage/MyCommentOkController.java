...
import com.sol.app.myPage.dao.MyCommentDAO;

public class MyCommentOkController implements Execute {

	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("MyCommentOkController 진입");

		String temp = request.getParameter("page");
		System.out.println("temp page : " + temp);
		int page = (temp == null) ? 1 : Integer.valueOf(temp); // 페이지 번호 기본값 1로 설정하겠다

		MyCommentDAO myCommentDAO = new MyCommentDAO();
		...
		if (memberNumber == null) {
			...
			int total = myCommentDAO.getTotal(memberNumber);
			...
			myCommentDAO.selectList(map).stream().map(gson::toJson).map(JsonParser::parseString).forEach((data) -> {
				JsonArray a = new JsonArray();

				a.add(data.getAsJsonObject().get("commentContent"));
				a.add(data.getAsJsonObject().get("reviewTitle"));
				a.add(data.getAsJsonObject().get("commentCreatedDate"));
