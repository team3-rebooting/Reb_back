...
import com.sol.app.myPage.dao.MySmallClubDAO;

public class MySmallClubOkController implements Execute {

	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("MySmallClubOkController 진입");
		...
		MySmallClubDAO mySmallClubDAO = new MySmallClubDAO();
		...
		if (memberNumber == null) {
			...
			int total = mySmallClubDAO.getTotal(memberNumber);
			...
			mySmallClubDAO.selectList(map).stream().map(gson::toJson).map(JsonParser::parseString).forEach((data) -> {
				//목록 행 정보
				JsonArray a = new JsonArray();

				a.add(data.getAsJsonObject().get("smallClubTitle"));
				a.add(data.getAsJsonObject().get("smallClubStartDate"));
				a.add(data.getAsJsonObject().get("smallClubCreatedDate"));
			...
			obj.addProperty("listTitle", "나의 개설 소모임 내역");
			obj.add("cols", cols);
			obj.add("list", list);
