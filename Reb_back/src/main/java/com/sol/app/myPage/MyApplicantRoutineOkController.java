...
public class MyApplicantRoutineOkController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("MyApplicantRoutineLoadOkController 진입");
		...
		if (memberNumber == null) {
			...
			int total = myRoutineDAO.getTotal(memberNumber);
			...
			myRoutineDAO.selectList(map).stream().map(gson::toJson).map(JsonParser::parseString).forEach((data) -> {
				...
				a.add(data.getAsJsonObject().get("routineTitle"));
				a.add(data.getAsJsonObject().get("routineLeaderName"));
				a.add(data.getAsJsonObject().get("routineMemberApplyDate"));
				a.add(data.getAsJsonObject().get("routineNumber"));
