...
import com.sol.app.club.dao.SmallClubCommentListDAO;

public class SmallClubCommentOkController implements Execute {

	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("SmallClubCommentOkController 진입");
		...
		SmallClubCommentListDAO smallClubCommentListDAO = new SmallClubCommentListDAO();
		...
		map.put("smallClubNumber", reviewNumber);
		
		// 페이징 정보 설정
		// BoardMapper.xml의 getTotal을 이용하여 전체 게시글 개수 조회
		// 실제 마지막 페이지 번호(realEndPage)를 계산함
		int total = smallClubCommentListDAO.getTotal(reviewNumber);
		...
		System.out.println(smallClubCommentListDAO.selectList(map));

		smallClubCommentListDAO.selectList(map).stream().map(gson::toJson).map(JsonParser::parseString)
				.forEach((data) -> {
					JsonObject o = new JsonObject();
					System.out.println(data);
					o.add("reviewNumber", data.getAsJsonObject().get("smallClubNumber"));
					o.add("reviewCommentNumber", data.getAsJsonObject().get("smallClubCommentNumber"));
					o.add("memberNumber", data.getAsJsonObject().get("memberNumber"));
					o.add("memberNickname", data.getAsJsonObject().get("memberNickname"));
					o.add("reviewCreatedDate", data.getAsJsonObject().get("smallClubCreatedDate"));
					o.add("reviewUpdatedDate", data.getAsJsonObject().get("smallClubUpdatedDate"));
					o.add("reviewContent", data.getAsJsonObject().get("smallClubContent"));
