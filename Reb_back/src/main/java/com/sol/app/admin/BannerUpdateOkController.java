...
public class BannerUpdateOkController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (up) {
			...
			if (lastBannerOrderNumber != 0) {
				int prevBannerNumber = bannerDAO.findBannerNumber(bannerOrderNumber - 1);
				bannerDTO.setBannerNumber(prevBannerNumber);
				bannerDTO.setBannerOrderNumber(bannerOrderNumber);
				bannerDAO.update(bannerDTO);

				AdminBannerDTO newBannerDTO = new AdminBannerDTO();
				newBannerDTO.setBannerOrderNumber(bannerOrderNumber - 1);
